...
describe("checkForUpdate", () => {
  ...
  it("reports update_available when GitHub's latest release is newer than the current version", async () => {
    (globalThis.fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
      json: async () => ({ tag_name: "v1.1.0", html_url: "https://example.invalid/releases/v1.1.0" }),
    });

    const result = await checkForUpdate("1.0.0", "owner", "repo");
    expect(result).toEqual({
      status: "update_available",
      currentVersion: "1.0.0",
      latestVersion: "1.1.0",
      releaseUrl: "https://example.invalid/releases/v1.1.0",
    });
  });
  ...
  it("reports check_failed on a non-ok HTTP response rather than throwing", async () => {
    ...
    const result = await checkForUpdate("1.0.0", "owner", "repo");
    expect(result).toEqual({ status: "check_failed", currentVersion: "1.0.0", latestVersion: null, releaseUrl: null });
  });
