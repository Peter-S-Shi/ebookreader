...
Last Updated: 2026-09-15 (V2 Launched — V2-M1 EPUB Reader Preference Override Compatibility In Progress)

Current Phase: **V2 Development — V2-M1 In Progress**. V1.0.0 remains released and its scope stays locked (see V1 Feature Freeze Decision Summary below, retained for history); the Human Feature Freeze Gate's requirement of explicit user authorization before further work has now been satisfied, opening V2. V2-M1 targets EPUB Reader Preference Override Compatibility only (no Book Hours, OCR, Notes, Authoring, or packaging scope). Root cause diagnosed via differential analysis of a real known-good and known-failing EPUB (private, gitignored, local-only fixtures — never committed); a withdrawn recovery patch (`441ce2f`, not merged, evidence-only) was audited and found to only partially and unreliably address the symptom. A corrected, minimal fix has been implemented on branch `v2-m1/epub-preference-override-compat` and passed targeted unit-test verification (`src/typography.test.ts`, `src/Reader.test.tsx`) plus manual verification against the real fixtures' extracted CSS (local-only, not committed). Full frontend/Rust CI and a human A/B retest on the real EPUBs are still outstanding — see V2-M1 section below for exact status. Do not treat V2-M1 as closed until those complete.
...
**V2-M1 EPUB Reader Preference Override Compatibility (In Progress — Implementation + Targeted Verification Complete, Full CI and Human A/B Retest Outstanding):**
...
- Regression mechanism now covered by public, synthetic, CI-safe unit tests in `src/typography.test.ts` (no private EPUB dependency): a publisher class applied to `<body>` itself, a publisher class applied directly to `<p>`, nested prose (`<blockquote><p>`) not compounding, and a chosen font-family plus a line-height override both winning together in the same rule.
- Targeted verification complete: `npx vitest run src/typography.test.ts src/Reader.test.tsx` (40/40 passed) and `npx tsc --noEmit` (0 errors). Also manually verified (local-only, not committed) against the real failing book's extracted CSS: font-size, margin, and padding all now respond correctly.
- **Outstanding before V2-M1 can be marked Complete:** full frontend test suite / typecheck / build (`npm test`, `npm run typecheck`, `npm run build`) and Windows Rust workspace tests/build via GitHub Actions CI at PR time (not run repeatedly during this dev loop, per instruction); and an explicit user human A/B retest on the two real EPUBs in a built/dev app, since a synthetic unit test cannot confirm real-world rendering fidelity.

Current Milestone: M1 — **Complete**, `94aff83`; M2 — **Complete**, `0a04fb4`; M3 — **Complete**, `096b90d` + durability test; M4 — **Complete**, `7f12173`; M5 — **Complete**, `67d32c5`; M6 — **Complete**, `9e99099`; M7 — **Complete**, `4d86045`; M8 — **Complete**, `2b81d54`; M9 — **Complete**, `5ccc240`; M10 — **Complete** (M10-A Complete [HUMAN PASS], M10-B Complete [HUMAN PASS], M10-C Complete [RELEASED]); V2-M1 — **In Progress** (implementation + targeted verification complete; full CI and human A/B retest outstanding).
Current Checkpoint: **V1_0_0_RELEASED_M10_COMPLETE_PP_COMPLETE_WORKSPACE_SLIMMING_COMPLETE / V2_M1_IN_PROGRESS**.
Current Branch / PR: `v2-m1/epub-preference-override-compat` (not yet pushed or opened as a PR).
...
Next Action: Push `v2-m1/epub-preference-override-compat`, open a PR, and let GitHub Actions CI run full frontend + Windows Rust verification; then request the user's human A/B retest on the two real private EPUBs before marking V2-M1 Complete.
