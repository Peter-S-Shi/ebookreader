...
Current Checkpoint / Promotion Unit: M2 checkpoint 1 — not yet started
Current Branch / PR: `main` (direct commits; no PR yet — single long-lived checkpoint per `ROADMAP.md` §1.1/§1.6)
Last Accepted Commit: `94aff83` (Needs Relink surfacing and Remove Book — CI green) on `main`
Last Accepted Evidence: V1 product/domain scope freeze + V1 UI baseline freeze + P0 loop-control refactor + M0 technical spikes across all 9 evidence tracks, strengthened by a corrective pass with a reproducible evidence harness (`tooling/m0-evidence/`); Architecture Lock approved at the second Human Architecture Gate — see `M0_TECHNICAL_SPIKE_REPORT.md`, `M0_ARCHITECTURE_DECISION.md`. **M1 complete**: Cargo workspace (`crates/domain` pure-Rust + `src-tauri` shell) with a live, CI-green Promotion Gate across seven consecutive checkpoints. 15 domain tests + 6 frontend tests: cryptographic fingerprinting; fingerprint-based duplicate-import detection; idempotent SQLite schema migration; Reference import (source untouched) and Managed-Copy import (bytes copied into managed storage); relink on unchanged fingerprint vs. explicit refusal on a changed one; Needs Relink surfaced when a Reference path goes missing; Remove Book (deletes a Managed Copy, never touches a Reference source — the identity/destructive-boundary evidence). Wired end-to-end: a real Tauri command surface (`import_book_command`, `list_library_command`, `relink_book_command`, `remove_book_command`) and a minimal Library view (native file picker via `tauri-plugin-dialog`, list, Needs Relink badge, Remove). Native visual acceptance confirmed directly: launched the live app (`npm run tauri dev`) and captured a foreground-verified native screenshot per the established visual-verification method, then exercised the real native "Open" file dialog (correct `*.epub;*.pdf;*.txt` filter) end-to-end. Full detail: `ROADMAP.md` M1 Exit Gate.
...
Architecture State: **Accepted Architecture Baseline** — see `M0_ARCHITECTURE_DECISION.md`. Amendments still require reconciliation under the Architecture Promotion Rule; Product-Contract Conflicts still require a Human Gate. Two M1 dev-environment findings, both isolated to this local sandbox and confirmed non-issues on CI's MSVC `windows-latest` runner (which remains authoritative regardless): (1) `cargo test` on this sandbox's GNU-hosted toolchain is unstable once Tauri's COM/WebView2 dependency graph links into a test binary (`STATUS_ENTRYPOINT_NOT_FOUND`) — worked around by keeping domain logic in the Tauri-free `crates/domain` member; (2) a real `cargo build` of the Tauri crate originally failed to *link* locally on this GNU-hosted toolchain (`ld: export ordinal too large`) — resolved (not just worked around) by dropping the unused `cdylib` crate-type (V1 has no mobile target), after which local `cargo build`/`cargo run`/`tauri dev` all link and run successfully.
Feature Complete: No
Feature Freeze: No
RC / Release State: Not started
Next Action: Begin M2 (Core Reading Across Formats) per its Success Evidence — representative EPUB/PDF/TXT documents reopen at stable locations, controls match actual format capability (`FORMAT_CAPABILITY_MATRIX.md`), font provenance model works, Reader/Focus pass native visual acceptance. This is a materially larger surface than M1 (real `foliate-js`/`pdf.js` renderer integration per the M0-evidenced architecture) — expect multiple checkpoints inside M2, per `ROADMAP.md` §1.1. No further per-Milestone human approval is required inside the M1→M8 envelope unless a stop/escalation trigger fires (`ROADMAP.md` §1.8); the next Human Gate is the Human Feature Freeze Gate after M8.
