...
Last Updated: 2026-09-10 (Post-UI Residual 1 — PDF Open Performance Optimization & Warm-Reopen Corrective Fix Complete — Awaiting Human Retest)

Current Phase: **Post-UI Engineering Residual Resolution — Residual 1 (PDF Open Performance Optimization & Corrective Fix) Complete**. C0/C1/C2 B1–B8 Canonical UI Migration is human-PASSed; Post-UI residual resolution is in progress:
1. B1–B8 Canonical UI Migration: Complete & Human-Accepted (PASS, commit `2b7724a`)
2. Post-UI Residual 1: PDF Open Performance Optimization & Warm-Reopen Corrective Fix (Complete, awaiting human retest)
...
- **Warm Reopen Saved-Location & RenderTask Fix (`PdfReader.tsx`)**: Resolved warm reopen rendering race condition where `setPdfDoc(pdf)` could trigger a single-page render effect before `load_reading_location_command` resolved `startPage`. Reordered state initialization so `startPage` resolves before `pdfDoc` state is populated. Added active `RenderTask.cancel()` in `useEffect` cleanup and handled `RenderingCancelledException`. Added regression test in `PdfReader.test.tsx`.
...
Current Checkpoint / Promotion Unit: **Post-UI Residual 1 PDF Open Performance Optimization & Warm-Reopen Corrective Fix Complete (Awaiting Human Performance Retest)**.
