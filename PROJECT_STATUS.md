...
Last Accepted Commit: `34a5620` (CI: real `cargo build` of the Tauri crate, not just `cargo check` — CI green) on `main`
Last Accepted Evidence: V1 product/domain scope freeze + V1 UI baseline freeze + P0 loop-control refactor + M0 technical spikes across all 9 evidence tracks, strengthened by a corrective pass with a reproducible evidence harness (`tooling/m0-evidence/`); Architecture Lock approved at the second Human Architecture Gate — see `M0_TECHNICAL_SPIKE_REPORT.md`, `M0_ARCHITECTURE_DECISION.md`. M1: Cargo workspace scaffolded (`crates/domain` pure-Rust + `src-tauri` shell); CI Promotion Gate live and green across three consecutive checkpoints (`84817f5`, `3551b24`, `aad7e56`). `crates/domain` has real TDD-built coverage (9 passing tests) for cryptographic fingerprinting, fingerprint-based duplicate-import detection, idempotent SQLite schema migration, and Reference/Managed-Copy import persistence (Managed Copy copies bytes into managed storage and leaves the user's source file untouched; Reference keeps the original path). This is wired to a real, reachable `import_book_command` Tauri command (`src-tauri/src/commands.rs`, `src-tauri/src/db.rs`), replacing the scaffold's placeholder "greet" command — the app now opens `library.sqlite3` in the Tauri app-data dir and applies migrations on startup.
...
Architecture State: **Accepted Architecture Baseline** — see `M0_ARCHITECTURE_DECISION.md`. Amendments still require reconciliation under the Architecture Promotion Rule; Product-Contract Conflicts still require a Human Gate. Two dev-environment findings carried forward (not architecture changes, both isolated to this local sandbox, both confirmed non-issues on CI's MSVC `windows-latest` runner): (1) `cargo test` on this sandbox's GNU-hosted toolchain is unstable once Tauri's COM/WebView2 dependency graph links into a test binary (`STATUS_ENTRYPOINT_NOT_FOUND`) — worked around by keeping domain logic in the Tauri-free `crates/domain` member; (2) a real `cargo build` (not just `cargo check`) of the Tauri crate fails to *link* locally in this GNU-hosted sandbox (`ld: export ordinal too large`, a mingw-ld limitation with Tauri's large symbol table) — CI's `cargo build -p ebookreader` on MSVC is unaffected and is the authoritative native Windows build verification (confirmed green, `34a5620`).
Feature Complete: No
Feature Freeze: No
RC / Release State: Not started
Next Action: Continue M1 implementation — relink workflow for missing/moved Reference files, a minimal Library view in the React frontend wired to `import_book_command` via `invoke()`, and native visual acceptance of that Library view — per M1's Success Evidence, still inside M1 checkpoint 1 unless scope/risk grows enough to warrant a second checkpoint. No further per-Milestone human approval is required inside the M1→M8 envelope unless a stop/escalation trigger fires (`ROADMAP.md` §1.8); the next Human Gate is the Human Feature Freeze Gate after M8.
